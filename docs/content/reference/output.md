...
describes what users get. Pick a format with `-o`, or let openalex choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
openalex <command> -o table   # aligned columns for reading
openalex <command> -o jsonl   # one JSON object per line, for piping
openalex <command> -o json    # a single JSON array
openalex <command> -o csv     # spreadsheet friendly
openalex <command> -o tsv     # tab-separated
openalex <command> -o url     # just the URL column
openalex <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
openalex <command> --fields id,title,url
```
...
```bash
openalex <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
openalex <command>            # a table, because this is a terminal
openalex <command> | wc -l    # JSONL, because this is a pipe
```
